  ...
  return (
    <div className="hidden items-center gap-2 rounded-xl border border-surface-border bg-surface-elevated/60 px-3 py-1.5 sm:flex lg:gap-3">
      <div className="flex items-center gap-1.5" title="TAO price (USD)">
        ...
        <span className="font-mono text-xs font-medium text-slate-700 dark:text-slate-200">{formatUsd(prices.taoUsd)}</span>
      </div>
      ...
      >
        ...
        <span className="font-mono text-xs font-medium text-slate-700 dark:text-slate-200">{formatUsd(prices.alphaUsd, 4)}</span>
      </div>
    </div>
  );
}
